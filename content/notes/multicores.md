...
OLTP Databses have lots of small transactions, therefore we can benefit by using multiple cores to execute these
...
Since only around 40% of the time is actually spent on computation, the rest is stalling

When stalling, the majority of this time it is handling L2 data or L1 instructions

Moore's Law still states the number of transistors doubles every 18 months, however clock speeds and poer have hit a wall

We can't get the energy and data in to the processor, or the heat out, at reasonable speed any more
...
Then more recently, we've moved to multisocket multicores
...
A setup using the research prototype Shore could potentially run 4 instructions per cycle, however, it only runs about 1 in practice

This is because around 70% of the time goes to stalls

Most of the time is waiting for data in order to continue execution
...
In practice, there is no penalty for L1, but anything lower is where you can get a possible stall
...
- next-line: if A is missed, we fetch A+1
- stream: if we miss A, A+1, we fetch A+2, A+3

This favours sequential access and spatial locality

However, it doesn't do well with instructions such as branches (we can use branch prediction for this) and function calls

It also doesn't help with data, as it doesn't do pointer chasing - to help with this, we can guess a stride if we have a predictable data layout in main memory
...
This takes advantage of the fact that programs follow the same flow repeatedly

The patterns of cache accesses are roughly the same on different inputs, and so if we know what the next cache access is most likely to be, we can prefetch it
...
We exploit recurring control flow

It's more accurate than light prefetching, however it comes at a higher space cost

### Software-Guided

In this case, we can exploit that we know a data structure being accessed
...
- Query compilation into machine/naive code
...
This means we make the most of each cache line, bu using as much of the data on it as possible
...
These are things like an index tree
...
Every time it calls next it gets a piece of data up the heirarchy and then gets it out to the result set

This leads to poor data & instruction cache locality

One way to fix this is to do vectorised execution, where we pull out the whole cache line on each call to `next()`
...
- We move the transaction to the core in which the data is
...
- We run one instruction per core
...
The second main problem is: LLC Data Misses
...
One way to reduce this is to use speculative lock inheritance

This means if $trxn_1$ uses some hot locks, and $trxn_2$ needs them, it can commit without releasing the hot locks and pass them to $trxn_2$
...
This is where we move from centralised locking to thread-local locking
...
We don't have a disk, so need no buffer management
...
We can do physiological partitioning (PLP)
...
If we assign a thread to work on a certain range of values of the tree, then there will be no conflicts, since the worker threads are always separate
...
We can do this with Aether Holistic Logging

There are three methods:

- Release the lock at the beginning of the commit phase

- We use flush pipelining, which is where we have a dedicated thread which is the log writer, which each thread flushes to
...
- We can also use a consolidation array to minimise log contention, where waiting threads combine their log requests and insert them together
...
Therefore, we make each socket an island, and we don't want to travel between them
