            ...
            {data.map((item, i) => {
                ...
                return(
                    ...
                    >
                        ...
                        <h2 className="vertical-timeline-element-title pad-2-b"><a href={data.url} className="link is-black"target="_blank" rel="noopener noreferrer">{data.title}</a></h2>

                        {data.shorthighlights.map((i) => (
                            <p className="vertical-timeline-element-badge"><span role="img" aria-label="star">⭐️</span> {i.highlight}</p>
                        ))}
                        {/* <p>{item.summary}</p> */}
